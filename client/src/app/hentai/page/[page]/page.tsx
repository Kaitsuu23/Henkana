// Re-uses the same layout/logic as the root hentai page but reads
// page number from URL params (/hentai/page/2/) instead of searchParams.
...
export const metadata: Metadata = { title: 'Hentai Anime' };

function pageHref(p: number) {
  if (p <= 1) return '/hentai';
  return `/hentai/page/${p}/`;
}

export default async function HentaiPageN({
  params,
}: {
  ...
  let hentaiData;
  try {
    hentaiData = await getHentai(page);
  } catch {
    hentaiData = null;
  }

  if (!hentaiData?.success) {
    return (
      <div className="py-20 text-center container mx-auto px-4">
        <h2 className="text-xl font-bold text-destructive">Error Loading Hentai</h2>
      </div>
    );
  }

  const { data, totalPages, hasNextPage, hasPrevPage, total } = hentaiData;
      ...
      <div className="relative overflow-hidden bg-muted/30 border-b border-border/40">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-3xl font-extrabold md:text-4xl tracking-tight">Hentai Anime</h1>
          </div>
          {total && (
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-1.5 pt-4">
                <PageArrow href={pageHref(page - 1)} isDisabled={!hasPrevPage} label="Previous">
                ...
                <PageArrow href={pageHref(page + 1)} isDisabled={!hasNextPage} label="Next">
