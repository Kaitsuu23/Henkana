...
function pageHref(p: number) {
  return p <= 1 ? '/2d' : `/2d/page/${p}/`;
}

export default async function Page2D({
  searchParams,
}: {
  ...
  if (!apiData?.success) {
    return (
      <div className="py-20 text-center container mx-auto px-4">
        <h2 className="text-xl font-bold text-destructive">Error Loading 2D</h2>
      </div>
    );
  }
