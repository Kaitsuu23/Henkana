import { getManhwa, getSidebar } from '@/lib/api';
...
export default async function ManhwaPageN({
  params,
}: {
  ...
  const [manhwaData, sidebarData] = await Promise.all([
    getManhwa(page).catch(() => null),
    getSidebar().catch(() => null),
  ]);

  if (!manhwaData?.success) {
    return (
      <div className="py-20 text-center">
        <h2 className="text-xl font-bold text-destructive">Error Loading Manhwa</h2>
      </div>
    );
  }

  const { data = [], totalPages, hasNextPage, hasPrevPage } = manhwaData;
        ...
        <div className="space-y-6">
          ...
          {totalPages > 1 && (
            <NumberedPagination
              ...
              hrefBuilder={(p) => p <= 1 ? '/manga/manhwa' : '/manga/manhwa/page/' + p + '/'}
            />
          )}
        </div>
