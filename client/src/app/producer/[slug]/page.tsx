...
import { getProducerSlug } from '@/lib/api';
...
export default async function ProducerSlugPage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>;
  ...
  let producerData;
  try {
    producerData = await getProducerSlug(slug, page);
  } catch (e) {
    producerData = null;
  }
    ...
    return (
      <div className="container mx-auto max-w-6xl px-4 py-24">
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/60 py-16 text-center">
          ...
          <h2 className="text-lg font-semibold text-foreground">Couldn't load this producer</h2>
          <p className="max-w-xs text-sm text-muted-foreground">
            Something went wrong fetching this page. Try refreshing, or head back to the producer list.
          </p>
          <Link href="/producer" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'mt-2')}>
  ...
  const { data, totalPages, hasNextPage, hasPrevPage, total } = producerData;

  // Format slug back to Title Case roughly for display
    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground uppercase tracking-widest">Producer</span>
            </div>
      ...
      {data && data.length > 0 ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/60 py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">No series found from this producer.</p>
        </div>
      )}
      </div>
    </div>
...
function ProducerPagination({
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 pt-4">
      <PageArrow href={page - 1 === 1 ? `/producer/${slug}` : `/producer/${slug}/page/${page - 1}/`} disabled={!hasPrevPage} label="Previous page">
      ...
      {pages.map((p, i) =>
        p === '…' ? (
          ...
          <Link
            key={p}
            href={p === 1 ? `/producer/${slug}` : `/producer/${slug}/page/${p}/`}
      ...
      <PageArrow href={`/producer/${slug}/page/${page + 1}/`} disabled={!hasNextPage} label="Next page">
...
// Builds a compact page list, e.g. 1 … 9 10 [11] 12 … 24
