...
import { getGenreSlug } from '@/lib/api';
...
export default async function GenreSlugPage({
  ...
  let genreData;
  try {
    genreData = await getGenreSlug(slug, page);
  } catch (e) {
    genreData = null;
  }
    ...
    return (
      <div className="container mx-auto max-w-6xl px-4 py-24">
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/60 py-16 text-center">
          ...
          <h2 className="text-lg font-semibold text-foreground">Couldn't load this genre</h2>
          <p className="max-w-xs text-sm text-muted-foreground">
            Something went wrong fetching this page. Try refreshing, or head back to the genre list.
          </p>
          <Link href="/genre" className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'mt-2')}>
  ...
  const { data, total } = genreData;
  const { totalPages, hasNextPage, hasPrevPage } = genreData.pagination || {};
    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground uppercase tracking-widest">Genre</span>
            </div>
      ...
      {data && data.length > 0 ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/60 py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground">No series found in this genre.</p>
        </div>
      )}
      </div>
    </div>
...
function GenrePagination({
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 pt-4">
      <PageArrow href={page - 1 === 1 ? `/genre/${slug}` : `/genre/${slug}/page/${page - 1}/`} disabled={!hasPrevPage} label="Previous page">
      ...
      {pages.map((p, i) =>
        p === '…' ? (
          ...
          <Link
            key={p}
            href={p === 1 ? `/genre/${slug}` : `/genre/${slug}/page/${p}/`}
      ...
      <PageArrow href={`/genre/${slug}/page/${page + 1}/`} disabled={!hasNextPage} label="Next page">
